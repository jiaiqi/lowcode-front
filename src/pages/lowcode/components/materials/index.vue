    ...
    <div class="left">
      <div class="type-list">
        ...
        <div
          ...
          v-for="(item, index) in filteredMaterialsTree"
          :key="index"
...
export default {
  ...
  computed: {
    ...
    current() {
      return this.filteredMaterialsTree[this.activeIndex];
    },
    filteredMaterialsTree() {
      if (this.mode === "mobile") {
        return this.materialsTree.filter((item) => item.value !== "layout");
      }
      return this.materialsTree;
    },
  },
  watch: {
    mode: {
      immediate: true,
      handler(newMode) {
        if (newMode === "mobile" && this.materialsTree[this.activeIndex]?.value === "layout") {
          this.activeIndex = 0;
          this.activeSubIndex = 0;
          this.comList = [];
          if (this.filteredMaterialsTree.length > 0) {
            this.getList(this.filteredMaterialsTree[0]);
          }
        }
      },
    },
